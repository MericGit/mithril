...
import { Routes, Route, useNavigate, useLocation } from 'react-router-dom'; // Import useNavigate and useLocation
import './App.css';
import './styles/Papers.css';
import Papers, { PapersRef } from './pages/Papers';
...
const App: React.FC = () => {
  ...
  const papersRef = useRef<PapersRef>(null);
  ...
  const handleUploadSuccess = (paper: any) => { // Handle paper upload success with navigation
    if (papersRef.current) {
      papersRef.current.addNewPaper(paper);
    }
    navigate('/papers');
  };
    ...
    <div className="app-container">
      <header className="app-header">
        ...
        <nav className="main-nav">
          ...
          <button 
            className={`nav-button ${location.pathname === '/papers' ? 'active' : ''}`}
      ...
      <Routes>
        ...
        <Route path="/papers" element={<Papers ref={papersRef} />} />
        <Route path="/upload" element={<PaperUpload onUploadSuccess={handleUploadSuccess} />} />
      </Routes>
    </div>
  );
};
